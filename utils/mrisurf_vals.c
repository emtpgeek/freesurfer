...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRISsampleAtEachDistance(MRIS *mris, int nbhd_size, int nbrs_per_distance)
{
...
int MRISsampleDistances(MRIS *mris, int *nbrs, int max_nbhd)
{
  int i, n, vno, vnum, old_vnum, total_nbrs, max_possible, max_v, vtotal;
  int *vnbrs, *vall, *vnb, found, n2, vnbrs_num, vall_num, nbhd_size, done;
      ...
      for (n = old_vnum; n < vall_num; n++) {
        ...
        if (vn->d >= UNFOUND_DIST / 2) {
          printf(
              ...
              n,
              vnb[n]);
          mrisCheckVertexVertexTopology(mris);
          DiagBreak();
          exit(1);
        }
...
/*-----------------------------------------------------
  ...
  Description
  ------------------------------------------------------*/
int MRIScanonicalToWorld(MRIS *mris, double phi, double theta, double *pxw, double *pyw, double *pzw)
{
  double x, y, z, radius;
